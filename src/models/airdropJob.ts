...
export class AirdropJobStore {
  ...
  findByRedeemCodeOrFiail(redeemCode: string): AirdropJob {
  ...
  // Retrieve a specific AirdropJob by uuid
  findByUuidOrFiail(uuid: string): AirdropJob {
    const result = this.data.find(job => job.uuid === uuid);
    if (!result) {
      throw new Error('Airdrop job not found');
    }
    return result;
  ...
  update(redeemCode: string, updatedJob: UpdateAirdropJobDto): AirdropJob {
    ...
    if (index !== -1) {
      const _airdropJob = { ...this.data[index], ...updatedJob, updatedAt: new Date() }
